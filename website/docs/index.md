...
Usage and cost are bucketed time series. Each row is one time bucket (`start_time`, `end_time`) and its `results` array holds the per-group breakdown, fanned out with `json_each` and read with `json_extract`. `start_time` is required and is Unix epoch seconds; `bucket_width` defaults to `1d`; `group_by` selects the breakdown dimension.
