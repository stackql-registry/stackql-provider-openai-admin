...
Phase 1 working notes for the `openai_admin` provider. Each item records what was investigated, the evidence, and what remains open. Sibling findings reused, not re-derived: anthropic_admin (the structural sibling - report row shape, array-param behaviour, cursor policies), the openai next-gen build (spec pin, list envelope, derived-cursor engine analysis, blocked-on-key pattern), openrouter (key-class documentation treatment), keycloak (REPLACE-vs-UPDATE warning). Reference pattern: stackql-provider-k8s (branch `feature/provider-dev`).
...
**Directory (the entity lists - 20 list operations).** Envelope from the pin: `{object: "list", data, first_id, last_id, has_more}` with `limit`/`after` request params (audit_logs adds `before`; projects adds `include_archived`). No dedicated next-token field - the derived cursor is the sibling's finding, reused not re-derived: `requestToken: {key: after, location: query}` fed from `responseToken: {key: $.last_id, location: body}`; `has_more` is not consumable by the engine (any-sdk `responseTerminator` exists in config vocabulary but nothing in the stackql loop reads it); termination happens via one empty overshoot request per full listing (`data: []`, `last_id` null/absent -> token extraction yields `<nil>`/`""` -> loop ends). Accepted cost, documented; the engine ticket for `responseTerminator` is the sibling's follow-up. Config placement is service-level `x-stackQL-config` (provider-level inheritance is broken in any-sdk - the k8s/jira/snowflake finding). Wire traversal is runbook probe 3.

Both idioms are classified per operation in `endpoint_inventory.csv` (`idiom` column: 9 bucketed, 20 directory, 52 single/action).
